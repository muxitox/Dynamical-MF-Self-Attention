...
def plotter(num_feat_patterns, tentative_semantic_embedding_size, positional_embedding_size, beta_list,
            ...
            keep_context, reverse_betas, beta_to_show, gaussian_scale_str):
    ...
    feats_reorder = [1, 2, 0]
    plot_save_plane(stat_results_beta_list_0[num_transient_steps:], stat_results_beta_list_1[num_transient_steps:,feats_reorder],
                    num_feat_patterns, max_sim_steps - num_transient_steps, show_max_num_patterns=num_feat_patterns,
                    save_not_plot=save_not_plot, tag_names=stats_to_plot, beta=beta_to_show)
...
if __name__ == "__main__":
    ...
    num_feat_patterns = 3
    num_transient_steps = 1500  # 0 if we want to show the trajectory since the beginning
    max_sim_steps = 4068
    keep_context = True
    reverse_betas = False
    beta_to_show = 0.6   # We'll find the nearest beta in the defined range
    ...
    plotter(num_feat_patterns, tentative_semantic_embedding_size, positional_embedding_size, beta_list,
            num_transient_steps, max_sim_steps, context_size, ini_token_idx, seed, normalize_weights_str,
            reorder_weights, save_not_plot, stats_to_save_plot, correlations_from_weights, se_per_contribution,
            keep_context, reverse_betas, beta_to_show, gaussian_scale)
